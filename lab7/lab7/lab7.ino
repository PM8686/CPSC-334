/*
    ...
    The node devices will broadcast random data to the primary device every 5 seconds and from time to time
    they will ping the other node devices with a "Hello!" message.

    The primary device will receive the data from the node devices and print it to the Serial Monitor. From time
    to time, the primary device will calculate the average of the priorities of the node devices and send it to
    all the node devices.

    Each device will have a priority that will be used to decide which device will be the primary.
    The device with the highest priority will be the primary.
    ...
    4. After all devices have been registered, the device with the highest priority will be the primary.
    5. The node devices will send random data to the primary every 5 seconds.
      - Every "REPORT_INTERVAL" messages, the nodes will send a message to the other nodes.
    6. The primary device will calculate the average of the data and send it to the node devices every "REPORT_INTERVAL" messages.
...
/*
    ...
    The Wi-Fi device maintains a Primary primary Key (PMK) and several Local primary Keys (LMK).
...
// Primary primary Key (PMK) and Local primary Key (LMK)
...
bool device_is_primary = false;       // Flag to indicate if this device is the primary
bool primary_decided = false;         // Flag to indicate if the primary has been decided
...
ESP_NOW_Network_Peer *primary_peer = nullptr;                           // Pointer to peer that is the primary
...
// Target MAC address for comparison
const uint8_t TARGET_MAC[] = {0x48, 0xe7, 0x29, 0x3f, 0xc2, 0x60};
...
void setup() {
  ...
  Serial.println("Setup complete. Broadcasting own priority to find the primary...");
...
void loop() {
  if (!primary_decided) {
    // Broadcast the priority to find the primary
    ...
    if (current_peer_count == ESPNOW_PEER_COUNT) {
      ...
      if (check_all_peers_ready()) {
        ...
        if (highest_priority == self_priority) {
          ...
          Serial.println("This device is the primary");
        } else {
          for (int i = 0; i < ESPNOW_PEER_COUNT; i++) {
            if (peers[i]->priority == highest_priority) {
              peers[i]->peer_is_primary = true;
              primary_peer = peers[i];
              Serial.printf("Peer " MACSTR " is the primary with priority %lu\n", MAC2STR(peers[i]->addr()), highest_priority);
              break;
            }
          }
        }
        Serial.println("The primary has been decided");
      } else {
    ...
    if (!device_is_primary) {
      ...
      if (!primary_peer->send_message((const uint8_t *)&new_msg, sizeof(new_msg))) {
        Serial.println("Failed to send message to the primary");
      } else {
        Serial.printf("Sent message to the primary. Hi this is Peter, Count: %lu, Data: %lu\n", new_msg.count, new_msg.data);
        sent_msg_count++;
      }
      ...
      if (recv_msg_count % REPORT_INTERVAL == 0) {
        ...
        for (auto &peer : peers) {
          ...
          if (!peer->send_message((const uint8_t *)&new_msg, sizeof(new_msg))) {
            ...
            Serial.printf(
              "Sent message to peer " MACSTR ". HERE this is Peter, Recv: %lu, Sent: %lu, Avg: %lu\n", MAC2STR(peer->addr()), recv_msg_count, new_msg.count, new_msg.data
            );
            sent_msg_count++;
          }
        }
